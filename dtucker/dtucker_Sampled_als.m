function [a, b, cores, ticktock, final_error, Y, er] = dtucker_Sampled_als(X, R_1, R_2, embedding_dims_1, embedding_dims_2, varargin)
...
for i = 1:N
    ...
    U = col(classical_mode_unfolding(re_cores{i}, 1));
...
for it = 1:maxiters
    ...
    for n = 1:N
        if n == 1
            ...
            U = col(classical_mode_unfolding(re_cores{1}, 1));
            sampling_probs{1} = sum(abs(U).^2, 2)/size(U, 2);
            ...
            U = col(classical_mode_unfolding(re_cores{n}, 1));
    ...
    samples = nan(J_2, N);
    sampling_probs_1 = cell(N,1);
    for i = 1:N
        rre_cores{i} = permute(cores{i},[2 1 3]);
        U = col(classical_mode_unfolding(rre_cores{i}, 1));
        sampling_probs_1{i} = sum(abs(U).^2, 2)/size(U, 2);
    end
    for m = 1:N
        samples(:,m) = randsample(sz(m), J_2, true, sampling_probs_1{m});
    ...
    for m = 1:N
        rescaling = rescaling ./ sqrt(sampling_probs_1{m}(samples(:, m)));
    end
