function [a, b, cores, total_time, nopre_time, final_error, varargout] = dtucker_SRFT_als(X, R_1, R_2, embedding_dims_1, embedding_dims_2, varargin)
...
%% Mixing bridge
cores_mixed = cell(N,1);
cores_mixed_1 = cell(N,1);
for i = 1:N
    cores_mixed{i} = double(ttm(tensor(re_cores{i}), diag_flips{i},1));
end
...
for it = 1:maxiters
    ...
    for n = 1:N
        if n == 1
            ...
            for m = 2:N
                ...
                core_samples{m} = cores_mixed{m}(samples(:,m), :, :);
            end
            ...
            Xn_sketch = Xn_sketch * (diag_flips{n}').';
    
            if isreal(X)
                G_sketch = [real(G_sketch)];
                Xn_sketch = [real(Xn_sketch)];
            end
            Z = (G_sketch \ Xn_sketch).';

            cores{n} = getcore(Z, a, b, n); 
            re_cores{1} = permute(cores{1},[2 3 1]);
            cores_mixed{1} = double(ttm(tensor(re_cores{1}),diag_flips{1},1));
        else
            ...
            cores_mixed{n} = double(ttm(tensor(re_cores{n}),diag_flips{n},1));
        end
    end
    %% updata A
    ...
    for i = 1:N
        ...
        cores_mixed_1{i} = double(ttm(tensor(rre_cores{i}), diag_flips{i},1));
    end
    for m = 1:N
        samples(:,m) = randsample(sz(m), J_2, true, sampling_probs{m});
        core_samples_1{m} = cores_mixed_1{m}(samples(:,m), :, :);
    end
    ...
    for m = 1:N
        samples(:,m) = randsample(sz(m), J_2, true, sampling_probs{m});
        core_samples_2{m} = cores_mixed{m}(samples(:,m), :, :);
    end
